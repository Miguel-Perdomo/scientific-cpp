// programa que imprima los sexy primes. 
#include<iostream>
#include<string>
#include<cmath>
...
int main(int argc, char **argv){// esto lo que hace es permitrirme leer cosas de la linea de comandos.
    ...
    for (int n = nmin; n+alpha <= nmax; n++){
        if(is_prime(n) == true and is_prime(n+delta)==true and is_prime(n+alpha)){
            std::cout << "(" << n << ", " << n+delta << ", " << n+alpha << ") ";
        }
    }
    std::cout << "\n";
}
